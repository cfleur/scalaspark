...
//    import sqlContext.implicits._
    ...
    */
    def cancelledDueToSecurity(df: DataFrame): DataFrame = {
        return df.select("FlightNum", "Dest").where($"CancellationCode"==="D")
    }
    ...
    */
    def longestWeatherDelay(df: DataFrame): DataFrame = {
        return df.filter($"Month" > "0").filter($"Month" < "4").agg(max("WeatherDelay"))
    }
...
//        ???
//        return spark.sql("SELECT Description from carriers INNER JOIN (SELECT UniqueCarrier FROM airTraffic GROUP BY UniqueCarrier) ON Code <> UniqueCarrier")
        val dft = spark.sql("SELECT DISTINCT UniqueCarriers FROM airTraffic")
        return spark.sql("SELECT Description from carriers INNER JOIN dft ON carriers.Code = dft.UniqueCarriers")
//        ???
//        Following 3 lines work but tests on assignment put constraints on main that do not allow for function call to take 2 args.
//        val cr = createOrReplaceTempView(carriers)
//        val tempdf = df.groupBy("UniqueCarrier").count()
//        val tempcr = tempdf.select("UniqueCarrier").join(cr, tempdf("UniqueCarrier") === cr("Code"), "inner")

//        people.filter("age > 30")
//          .join(department, people("deptId") === department("id"))
//          .groupBy(department("name"), "gender")
//          .agg(avg(people("salary")), max(people("age")))
//        return temp.join(carriers("description"), temp("UniqueCarrier") === carriers("Code")))
//        return tempcr.select("Description")
//        val dft.registerTempTable("df")
//        val carrierst.registerTempTable("carriers")
//        return sqlContext.sql("SELECT description FROM carriers INNER JOIN (SELECT DISTINCT UniqueCarriers FROM df) ON Code <> UniqueCarrier")
//        return carriers.schema
//        return carriers.select("Description").join(carriers, df("UniqueCarrier").groupBy("UniqueCarrier") === carriers("Code"))
    }
