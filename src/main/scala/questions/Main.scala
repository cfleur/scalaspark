...
object Main {
  ...
  def main(args: Array[String]) = {
    ...
    val airTraffic = processor.loadDataAndRegister(filePath)
    val carriers = processor.loadDataAndRegister(carriersPath)

    println(airTraffic.schema)
//    airTraffic.printSchema()
//    data.collect().foreach(println)
     println("<<<security>>>")
     processor.cancelledDueToSecurity(airTraffic).show()
     println("<<<weather dealy>>>")
     processor.longestWeatherDelay(airTraffic).show()
     println("<<<didn't fly>>>")
     processor.didNotFly(airTraffic).show()
